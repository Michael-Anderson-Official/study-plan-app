...
- 節気ごとの背景写真（ユーザーがAI生成、文字なし）を追加した。`img/sekki/*.jpg`（900x1600・約150〜300KBに圧縮）を置いて`SEKKI_IMAGES`（節気名→パス）に登録すると、その節気では白半透明(0.78)を重ねた全画面背景として`applySekkiPhoto()`が表示し、SVGモチーフは省略する。未登録の節気は従来のSVGモチーフのまま。節気名自体は画像に焼き込まず`#sekkiLabel`側で表示する方針（二重表示・誤字防止のため）。全画面スプラッシュ表示は2026-07-08に削除済み（15日に1回・数秒しか見えず実装コストに見合わないと判断）。`?sekki=立春`のようにURLへ付けると任意の節気をプレビューできる。現在は立春〜小雪の20枚（`risshun`/`usui`/`keichitsu`/`shunbun`/`seimei`/`kokuu`/`rikka`/`shouman`/`boushu`/`geshi`/`shousho`/`taisho`/`risshuu`/`shosho`/`hakuro`/`shuubun`/`kanro`/`soukou`/`rittou`/`shousetsu`）。初回の文字入り立春・雨水画像は2026-07-08に文字なし版へ差し替え済み。春分（満開すぎて清明と似ていた）・芒種（季節的に不自然な桜が写っていた）の2枚も指摘を受けて2026-07-08に差し替え済み。残り4枚（大雪・冬至・小寒・大寒）はユーザーが順次生成予定。
