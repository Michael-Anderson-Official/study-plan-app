...
最終更新: 2026-07-08（CodexがGoogleカレンダーの認可コードフロー + Worker refresh token保管を実装し、Worker本番デプロイとCloudflareの `GOOGLE_OAUTH_CLIENT_SECRET` 設定まで完了。実ブラウザでの初回認可と自動更新は未確認）
...
- 実カメラでのバーコード読み取り。
- 日付セルタップ→日別詳細パネル表示、日別パネル内の○/×、教材詳細ボタン、Google予定追加ボタン、振り返りtextareaの実ブラウザ操作。
...
- PC/スマホでの最新UIの実操作。
- 1週間タブの日付セルタップで日別詳細パネルを開く新UIの実ブラウザ操作（検証環境の `playwright-core` 不足により自動クリック確認は未実施）。
- 実カメラでのISBNバーコード読み取り。
...
- **Googleカレンダーのサインインが約1時間で切れる問題への対応（2026-07-08に認可コードフロー方式を実装）。**
  ...
  - 未確認: 実ブラウザでの初回認可コード交換、1時間後または強制期限切れ時の自動更新。
