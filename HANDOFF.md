...
- **計画中（2026-07-12ユーザー発案・庭の完成後に着手）: 「まち」キャンバス**。庭と並ぶ第2のステッカーキャンバスとして現代の街を追加し、勉強×まちづくりゲームにする。無料ステッカー=マンション・コンビニ・病院・信号・公園・銭湯など、プレミアム=東京タワー・スカイツリー・雷門・観覧車・城などのランドマーク。獲得時にユーザーが庭/街どちらのプールから引くか選ぶ設計（ランダム混合はコレクションが薄まるので不可）。実装は既存の庭エンジンを汎用化（ステッカーとgarden-itemsに`canvas`区分を追加＋背景切り替え）するだけで作り直し不要。**着手条件: 庭の背景24枚とステッカー画像が揃い、TestFlightで庭が検証されてから。**
