...
- 節気ごとの背景写真（ユーザーがAI生成）を追加した。`img/sekki/*.jpg`（900x1600・約200KBに圧縮）を置いて`SEKKI_IMAGES`（節気名→パス）に登録すると、その節気では白半透明(0.78)を重ねた全画面背景として表示し、SVGモチーフは省略する。未登録の節気は従来のSVGモチーフのまま。節気が切り替わって最初に開いたときは`maybeShowSekkiSplash()`が写真を全画面スプラッシュ表示する（既視判定は端末ローカルの`sekki-splash-seen`キー、クラウド同期対象外）。`?sekki=立春`のようにURLへ付けると任意の節気をプレビューできる（プレビュー中はスプラッシュも毎回出る）。現在は立春・雨水の2枚のみ。残り22枚はユーザーが順次生成予定。注意: 立春の画像は焼き込まれた七十二候テキストに誤りがある（蟄虫始振は啓蟄の候）ため、いずれ文字なし版への差し替えを推奨。
...
- 科目1〜7のセルは直接編集を廃止した（`contenteditable`を外した）。タップで○✕切り替えの吹き出し（`#statusBubble`）が出る。空セル・折りたたみ行では出ない。今日以前の日付に新しく✕を付けると後ろ倒しの確認ポップアップが出る。8列目（感想・振り返り）だけは`contenteditable`のまま。
