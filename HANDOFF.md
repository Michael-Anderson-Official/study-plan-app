...
最終更新: 2026-07-08（Claude Codeが二十四節気の背景写真を全24枚完成、Googleカレンダーのサインイン期限切れ対応を検討中）
...
- **Googleカレンダーのサインインが約1時間で切れる問題への対応（2026-07-08、ユーザーから改善依頼あり。まだ未着手）。**
  - 現状: `initGoogleAuth()`/`requestGoogleToken()`はGIS「トークンクライアント」（暗黙的フロー）を使っており、アクセストークンのみでリフレッシュトークンを持たない設計。トークンは`google-token`に`{token, expiresAt}`で保存され約1時間で失効し、以後は手動での再サインインが必要。
  - 検討した2案:
    - **A. サイレント再認証（簡易）**: ページ読み込み時などに`requestGoogleToken({prompt:''})`を自動的に試みて、ユーザー操作なしでトークン更新を狙う。実装は軽いが、**iPhone SafariのITP（ストレージ分離）により動かない可能性が高い**。根拠: 本セッションで`signInWithPopup`/`signInWithRedirect`（Firebase Auth）が全く同じ理由（`firebaseapp.com`とのクロスドメインのセッション/ストレージ受け渡しがSafariにブロックされる）で実機失敗し、最終的にGISのサインインボタン＋`signInWithCredential`方式に切り替えて解決した経緯がある（本ファイルの「アカウント同期（Firebase）」節参照）。サイレント再認証もブラウザのGoogleセッションCookieに依存する点で同種のリスクを抱える。
    - **B. 認可コードフロー＋Worker側でリフレッシュトークン保管（推奨・未実装）**: `google.accounts.oauth2.initCodeClient()`で認可コードを取得し、Workerの新エンドポイント（例: `/google/token`）に送ってサーバー側でGoogleのトークンエンドポイントとやり取りする。リフレッシュトークンはクライアントに渡さずWorker側（KV、`sub:`と同様にuid単位）で保管し、以後はWorkerに「新しいアクセストークンをください」と頼むだけにする。ユーザー側の準備としてGoogle Cloud ConsoleでOAuthクライアントシークレットの発行が必要（VAPID/Firebase設定と同様の一度きりの作業）。Cookie/セッションに依存しないため、Safari ITPの影響を受けずに確実に解決できる見込み。
  - ユーザーはまだA/Bのどちらで進めるか決定していない。着手前に必ず確認すること。
