...
- 2026-07-12、街の視点を**アイソメトリック（2:1ダイヤ型マス・シムシティ型）**に変更した（ユーザー要望）。データは(col,row)のまま//で投影・逆投影し、地面とマス目はSVG data URIで描画、zIndex=col+rowの画家のアルゴリズムで奥行きを出す。ドラッグのスナップは足元座標の逆投影で判定（検証済み）。建物画像の生成指定はアイソメ視点に変更（MACHI_DESIGN.md更新済み）。
