...
- 2026-07-20、「記憶効率が最大化される復習ペースで自動的に書き込まれるようにしたい」との要望で、**間隔反復（忘却曲線）ベースの自動復習スケジュール**を追加した。
  - UI: 教材詳細の「一日あたりの目標単位数」の下にチェックボックス「🔁 復習も自動で組み込む」（`#autoReviewInput`）。`material-details`の`autoReview`（true/null）として保存し、開き直すと復元される。既存教材はOFFのまま（opt-in）。
  - 動作: 「保存」（`scheduleMaterial`）・「毎日」（`scheduleMaterialDaily`）の全4ブランチ（目次/数値×期日あり/なし）が、割り当てを一旦`plan`（iso→内容行配列）に集め、共通の`writeScheduleCells(idx, plan, autoReview)`で書き込む形にリファクタ。autoReview時は各学習日の内容行ごとに`REVIEW_INTERVALS = [1, 7, 30]`日後（翌日・1週間後・1ヶ月後）のセルへ「`🔁復習: <内容>`」行を追記する（新規学習と同じ日に当たれば同セルにマージ）。復習だけの日はセルが復習行のみになる。
  - 進捗表との連動: `matchUnitsFromContent`が行頭の「🔁復習: 」プレフィックスを外してから照合するようにした（数値モードも行ごとに分割してから`,`区切りをパース）。これにより**復習日に○を付けると進捗表の「次の空いている周」（2周目以降）に日付が記録され**、取り消しも既存の`unrecordProgressFromDay`の対称ロジックで1件だけ消える。
  - あわせて既存の小さな不整合を修正: スケジュール保存直後にモバイルの日付カード（`#mobileWeekStrip`の「n件/予定なし」）が古いままだった→`updateCellsForCurrentWeek()`末尾に`renderMobileWeekSelector()`を追加。
  - 確認状況: scratchpadのplaywright-core＋ローカルEdge（headless）で22項目の自動テストがPASS（目次モードの配置（翌日/7日/30日）・数値モード＋期日あり・OFF時は従来通り・チェック復元・復習○→u0-l2記録→取り消し・日付カード即時反映・スクショ目視）。実機未確認。復習間隔[1,7,30]は`REVIEW_INTERVALS`定数で変更可。
- 2026-07-13、街を**シムシティ路線に大幅強化**した（ユーザー指示「完全にシムシティに寄せたい」→AskUserQuestionで全項目選択）。
  - **人口システム**: カタログ全建物にpop値（自宅4、コンビニ12、マンション60、学校40…プレミアムは120〜250）。**道路に面した建物はにぎわいで人口2倍**（machiNearRoad/machiPopulation）。ヘッダーに「人口 ○○人 ・ 建物 ○件」、建物タップのチップにも人口とにぎわい状態を表示。道路が飾りから都市計画になった。
  - **マップ拡張**: 建物6件ごとに16×10から+4列+2行（最大36×20）。建物数から決定的に導出（updateMachiGridSize、保存・同期不要で端末間一致）。タイルサイズは基本グリッド基準で固定なので、拡張するとマップが物理的に広がる。獲得時にしきい値を跨ぐと「🌆 人が集まってきた！街が広がりました」チップ。
  - **道路を走る車**: 道路3マス以上で出現（10マスごとに+1台、最大4台）。SVGの小さな車が道路セルの中心をランダムウォーク（直前のセルには戻らない、行き止まりは折り返し）。rAFループはビュー非表示で自動停止・openMachiViewで再開（machiCarTick/startMachiCars）。zIndex=9+col+rowで建物の奥行きに混ざる。
  - **昼夜の色**: #machiDaylightオーバーレイ（updateMachiDaylight、5-7時朝焼け/16-18時夕方/18-20時たそがれ/20-5時夜の青）。60秒ごと更新。ヘッダーは染めない（scroller.offsetTopから下だけ）。テスト用にupdateMachiDaylight(強制時刻)可。
  - **地面の装飾**: セルごとの緑トーン差＋草むら＋小花を座標ハッシュで決定的に散布（machiGroundMarkup内。再描画・拡張でも同じ場所、チラつかない）。イラスト画像化はグリッド可変と相性が悪いためプロシージャルSVGを採用。
  - やらないと決めたこと（ユーザー同意の設計方針）: 経済・税金シミュ（勉強だけが進行源の原則を壊す）、災害・衰退（罰を軽くする思想と矛盾）、本格交通シミュ（車は飾り）。
  - 確認状況: ローカルPlaywrightで人口計算（88人=4×2+12×2+60）・にぎわいチップ・拡張（12件で24×14、拡張域配置可・境界外拒否・地面が物理的に拡大）・車の出現と走行・ビュー閉で停止・夜の色・地面装飾を確認。長押し/道路/ズームの既存スイートも全て再実行しパス。
- 2026-07-13、**建物を動かすと変な影（縞状の残像）が残る**iOS実機バグを修正した（ユーザーがスクショで報告）。
  - 原因: 地面をCSS background-image（巨大SVGのdata URI、2000px超）で描いていて、長押し時のマス目強調・道路更新のたびにURI全体を差し替えていた。iOS Safariはこの巨大背景の再ラスタライズで古いタイルのゴミ（道路が縦に滲んだような縞）を画面に残すことがある。
  - 対策: 地面・マス目・道路を**実SVG要素**（#machiGroundSvg、machiRenderGround()がinnerHTMLを更新）に変更。machiGridBackground→machiGroundMarkup（内側マークアップ生成）＋machiRenderGround（要素管理）に分割。data URIのencodeURIComponentも不要になり道路なぞり時の再描画も軽くなった。旧CSSのプレースホルダー格子（.machi-groundのlinear-gradient）も削除。
  - Chromiumでは再現しない事象のため、**実機での残像消滅はユーザー確認待ち**。既存テスト（長押し7項目・道路/ズーム一式）は全て再実行しパス。
- 2026-07-13、自宅スプライトとマス目のズレを修正した（ユーザー指摘）。原因は透過余白＋アンカー概算。対策:
  - home.webpを不透明領域でトリミング（768²→718×578）。
  - 建物画像の整合を**実測メタデータ駆動**にした: カタログの imgScale（表示幅=tileW×w×imgScale。絵の底辺ダイヤ幅がフットプリント幅に一致するよう算出）/ imgBaseX（絵の底の頂点のX割合。ここがフットプリントの底の頂点に来るようtranslateXで補正）/ imgFringe（白フチの厚みぶん沈める割合）。elは translate(-50%,-100%) でアンカー=フットプリント中心、img側のtranslateで底の頂点をtileH×h/2下の底コーナーへ落とす。自宅の実測値: imgScale 1.12 / imgBaseX 0.586 / imgFringe 0.014。
  - 計測手順（新しい建物画像を受け入れるとき毎回）: canvasでアルファ>10のbboxを取りトリミング→底からdyごとの不透明幅をスキャンして底辺ダイヤの頂点X・左右の角・勾配を確認→imgScale=フットプリント幅÷絵の底辺幅、imgBaseX=底の頂点X割合→半透明ゴースト＋machiShowCellHighlightのスクショで目視確認。この家の絵の底辺勾配は左2.1:1・右1.54:1と非対称だったため、頂点基準が最も自然だった。
  - 長押し・道路・ズームの既存テストは全て再実行しパス。
- 2026-07-13、街の建物操作を**長押し移動**に変更した（「触ると動いてしまい不便」とのユーザー指摘）。
  - タップ=今まで通り（自宅→庭、他→思い出チップ）。**触ってすぐ動かす=パン**（建物は動かない）。**長押し450ms=持ち上げて移動モード**（.draggingで浮いて1.06倍、navigator.vibrate(15)、iOSの長押しメニューは-webkit-touch-callout:noneで抑止）。
  - 移動中は machiGridBackground(iso, roads, true) でマス目線を濃く太く強調し、スナップ先のフットプリントを machiShowCellHighlight（#machiCellHighlight、SVGひし形、z-index:5）で**緑=置ける/赤=置けない**表示。置けないマスで離すと差し戻し。置けないマスの判定は持ち上げ時にキャッシュ（自分以外の建物＋道路）。
  - 確認状況: ローカルPlaywrightで、即動かす=パン・タップ=チップ・長押しリフト・緑/赤切り替え・差し戻し・空きマスへのスナップ保存・自宅タップで庭、の7項目を確認。実機の長押し感触（450ms）は未確認、好みで調整余地あり。
- 2026-07-13、街に**ピンチズーム**と**道路**を実装した。
  - **ズーム**: machiZoom（0.6〜2.5、localStorage machi-zoom・端末ローカルで同期しない）をmachiIsoParamsのタイルサイズに乗算。setMachiZoom(zoom, anchorX, anchorY)がピンチ中心の下の街の場所を固定したまま再レイアウト（renderMachiGrid(true)=keepScrollで再センタリングを抑止し、スクロール位置を比率で再計算）。2本指ピンチ（rAFスロットル）とPCのCtrl+ホイールに対応。
  - **ジェスチャーエンジン**: #machiScrollerをtouch-action:noneにして、タッチのパンもJS側で統一処理（1本指=パン、2本指=ピンチ、ピンチ後に1本残ればパン継続）。建物ドラッグは.machi-item側がpointer captureするので衝突しない。setPointerCaptureはtry/catchで包む（合成イベント・離指後発火対策）。
  - **道路（無料・無制限）**: ヘッダーの🛣️ボタン（#machiRoadBtn）で道路モードをトグル。1本指でマスをなぞって敷く／道路の上からなぞり始めると消しゴムになる。建物のあるマスには敷けず、道路のあるマスに建物は置けない（machiCanPlaceが両方見る）。データはmachi-roads（"col,row"配列）でapp/settingsに同期（classifyStorageKey/pushSettings/applyCloudにmachiRoads追加）。描画はmachiGridBackgroundのSVGに路面ひし形＋隣接道路への白破線センターラインを直接描く（DOM増えない・zIndexの心配なし）。openMachiViewは常に通常モードで開く。
  - 確認状況: ローカルPlaywright（合成ポインタ）でピンチ拡大縮小・ズーム保存・1本指パン・なぞり敷設5マス・消しゴム・自宅マス拒否・道路上の建物拒否・リロード後の永続を確認。実機のピンチ感触は未確認。
- 2026-07-13、自宅の本物スプライトを登録した（img/machi/home.webp、768px・75KB）。和風平屋（瓦屋根・漆喰壁・障子・縁側・引き戸）。透過検証済み（55%透過・四隅アルファ0）。**以降の建物はこの自宅を参照画像にしてChatGPTに「同じ角度・同じタッチで」と依頼する**（アイソメ斜め上45度・屋根上面＋正面＋右側面・地面と影を描かない・白フチ）。洋風二階建ての初稿も出来が良いので、将来1×1無料建物「ようふうのいえ」候補として保管（アップロード履歴にあり、未登録）。スターターを平屋にしたのは課金アップグレード（平屋→二階建て→和風邸宅）の伸びしろのため。
- 2026-07-13、街の建物カタログに画像対応を追加した（庭と同じ流儀）。MACHI_ITEM_CATALOGにimgフィールドがあれば buildMachiItemEl が絵文字の代わりに<img>（幅=フットプリントのマスw個ぶん、.has-imgで足元アンカー-94%）を描画し、ステッカー帳のまちセクションもサムネ表示する。ダミー画像でPlaywright確認済み（描画・自宅タップで庭・ドラッグスナップ・帳サムネ）。実画像はユーザーがChatGPTで生成中（自宅から。以降の建物は自宅を参照画像にして同一角度を保つ）。
