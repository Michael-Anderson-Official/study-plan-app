...
最終更新: 2026-07-08（CodexがGoogleカレンダーの認可コードフロー + Worker refresh token保管を実装し、Worker本番デプロイまで完了。Cloudflareの `GOOGLE_OAUTH_CLIENT_SECRET` 設定が未完了）
...
- WorkerのGoogleカレンダー長期連携にはCloudflare側で `GOOGLE_OAUTH_CLIENT_ID`（変数）と `GOOGLE_OAUTH_CLIENT_SECRET`（secret）の設定が必要。`GOOGLE_OAUTH_CLIENT_ID` は `worker/wrangler.toml` の `[vars]` に追加済み。`GOOGLE_OAUTH_CLIENT_SECRET` は2026-07-08時点で未設定。client secretとrefresh tokenはリポジトリやブラウザに置かない。
...
- Cloudflare側の `GOOGLE_OAUTH_CLIENT_SECRET` が本番で未設定。公開Workerの `/google/token` はHTTP 503を返すところまで確認済み。
- Worker logsでscheduled eventが毎分動いているかは未確認。

危険箇所:

- push送信に失敗しても `lastSentDate` を書くため、その日は再送されない（購読ごとの状態になったが、失敗時に再試行しない挙動自体は変わっていない）。
- Worker本体は2026-07-08にWranglerで本番デプロイ済み（Version ID: `7d320142-2306-4dc2-b633-9abeb228baf1`）。ただし `GOOGLE_OAUTH_CLIENT_SECRET` が未設定のため、Googleカレンダー長期連携はまだ実用状態ではない。
...
- 公開Workerの `/google/token` は `GOOGLE_OAUTH_CLIENT_SECRET` 未設定のためHTTP 503を返す。
...
- **Googleカレンダーのサインインが約1時間で切れる問題への対応（2026-07-08に認可コードフロー方式を実装）。**
  ...
  - 完了済み: Cloudflare Worker本番デプロイ、`GOOGLE_OAUTH_CLIENT_ID` 変数追加。
  - 未完了: Cloudflare Worker secret `GOOGLE_OAUTH_CLIENT_SECRET` の設定。
  - 未確認: Google Cloud Console側のOAuthクライアントシークレット発行/設定、実ブラウザでの初回認可コード交換、1時間後または強制期限切れ時の自動更新。
