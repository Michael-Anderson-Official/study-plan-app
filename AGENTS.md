...
- 目的: 1週間の学習計画、教材リスト、ISBN/バーコード教材追加、週の振り返り、Googleカレンダー予定、毎日通知を管理する静的Webアプリ。将来的にAppStoreで公開配信することを見据えており、そのための土台としてFirebaseによるアカウント同期を導入済み（詳細は「アカウント同期（Firebase）」節を参照）。
- 収益化方針（2026-07-12ユーザー決定）: **将来的に広告を載せて収益化する**。マーケティング文言や設計提案で「広告なし」を売りにしないこと。UI設計では将来の広告枠が入る余地を考慮してよい。ターゲット層は受験する中高生と資格試験の大学生・社会人（初期は資格試験の社会人に絞る想定）。2026-07-08、「今日やること」「Googleカレンダー」の各タブは廃止し、1週間タブ1本（日付タップで開く日別詳細パネル）＋右上の通知ベル（○✕未チェックの過去分）に統合した。
...
- 「今日やること」「Googleカレンダー」タブは2026-07-08に廃止した。日付セル（行ラベル）タップで開く日別詳細パネル（`#weekDayPanel`、`renderWeekDayPanel()`）がその日の教材予定・○✕・Google予定・感想欄をまとめて表示する。○✕が付いていない過去の予定は、右上の通知ベル（`#notifBellBtn`→`#notifModal`、`pastPendingTasks`/`refreshPastPendingTasks()`/`renderNotifList()`/`updateNotifBadge()`）にまとめた。バッジ件数とモーダル内リストは○✕が変わった瞬間に`onTaskStatusChanged()`で即時再計算する（2026-07-12変更。旧仕様の「ベルを開いたときだけ再計算」はやめた）。
- Googleカレンダーのサインイン/今すぐ同期ボタンは設定モーダルの「Googleカレンダー連携」セクションに移設した（アカウント同期＝Firebaseとは別セクション）。「今日は何の日」（`#todayTrivia`）はタイトル下に常時表示、モバイル幅では非表示（`body.week-mobile-shell #todayTrivia { display:none; }`）。
- カレンダー表示は複数選択に対応済み（2026-07-08）。`googleSelectedCalendarIds`配列（`localStorage`の`google-calendar-ids`、JSON配列文字列）が表示対象カレンダーIDの一覧。設定モーダルのチェックボックス一覧（`#googleCalendarCheckboxList`）で選択する。`fetchGoogleEvents()`は選択中の全カレンダーへ並行fetchし、各予定に取得元カレンダーID（`ev._calendarId`）を付与してマージする。既存予定の編集・削除は`ev._calendarId`宛てに行う（`currentEditingGoogleCalendarId`）。新規作成時の追加先は`googleEventCalendarSelect`（書き込み可能なカレンダーのみ、デフォルトprimary）で個別に選ぶ。
